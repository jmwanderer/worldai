...
def save_character_thread(db, session_id, wid, cid, thread):
  ...
  """
  now = time.time()
  c = db.execute("SELECT thread_id FROM character_threads WHERE " +
  ...
  if r is not None:
    ...
    db.execute("INSERT INTO character_threads VALUES (?, ?, ?, ?)",
...
def delete_character_thread(db, session_id, wid, cid):
  ...
  if r is not None:
    thread_id = r[0]
    db.execute("DELETE FROM character_threads WHERE " +
               "session_id = ? AND world_id = ? AND character_id = ?",
               (session_id, wid, cid))
    db.execute("DELETE FROM threads WHERE id = ?",
               (thread_id,))
  db.commit()
