...
def thread_api(wid, cid):
    ...
    Returns an extended chat.ChatResponse or {"error":...}
    Extended with:
    - enabled: can user continue to chat with character
    """
    session_id = get_session_id()
    wstate_id = world_state.getWorldStateID(get_db(), session_id, wid)
...
def action_api(wid, cid):
    ...
    Returns an extended chat.ChatResponse
    Adds:
      - message: message generated by the action
      - changed: state of the world changed as a result
      - enabled: user can continue to chat with character
    """
    session_id = get_session_id()
    world = elements.loadWorld(get_db(), wid)
    if world is None:
    ...
    if len(event) > 0:
        ...
        result = chat.ChatResponse(id=os.urandom(8).hex(), done=True)
